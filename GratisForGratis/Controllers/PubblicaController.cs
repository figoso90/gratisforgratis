...
namespace GratisForGratis.Controllers
{
    public class PubblicaController : AdvancedController
    {
            ...
            if (ModelState.IsValid)
            {
                using (DatabaseContext db = new DatabaseContext())
                {
                    using (DbContextTransaction transaction = db.Database.BeginTransaction())
                    {
                        try
                        {
                            PERSONA utente = (Session["utente"] as PersonaModel).Persona;
                            Guid token = Guid.Parse(viewModel.TokenOK);
                            ANNUNCIO annuncio = db.ANNUNCIO.SingleOrDefault(m => m.TOKEN == token && m.ID_PERSONA != utente.ID);
                            if (annuncio != null)
                            {
                                // controllo se ha già quell'annuncio
                                ANNUNCIO annuncioInPossesso = db.ANNUNCIO.FirstOrDefault(m =>  m.ID_PERSONA == utente.ID 
                                ...
                                {
                                    ANNUNCIO_DESIDERATO annuncioDesiderato = db.ANNUNCIO_DESIDERATO.Where(m => m.ANNUNCIO.TOKEN == token && m.ID_PERSONA == utente.ID).SingleOrDefault();
                                    ...
                                    if (viewModelAnnuncio.SalvaAnnuncio(ControllerContext, annuncioInPossesso))
                                    {
                                        ...
                                        if (idAnnuncio != null)
                                        {
                                            annuncioInPossesso.PERSONA = utente; // perchè sennò non riesce a recuperare l'associazione
        ...
        private ActionResult SaveAnnuncio(PubblicazioneViewModel model)
        {
            ...
            using (DatabaseContext db = new DatabaseContext())
            {
                ...
                using (DbContextTransaction transaction = db.Database.BeginTransaction())
                {
                    try {
                        ...
                        if (ModelState.IsValid && model.SalvaAnnuncio(ControllerContext, annuncio))
                        {
            ...
            string nomeVistaDettaglio = GetNomeVistaDettagliAnnuncio(model.CategoriaId);
            if (!string.IsNullOrWhiteSpace(nomeVistaDettaglio))
            {
                ViewData["infoAggiuntive"] = RenderRazorViewToString(nomeVistaDettaglio, model);
            }
            else
            {
                string nomeTipologiaDettaglio = GetNomeVistaTipologia(model.CategoriaId);
                if (!string.IsNullOrWhiteSpace(nomeTipologiaDettaglio))
                    ViewData["infoAggiuntive"] = RenderRazorViewToString(nomeTipologiaDettaglio, model);
            }
